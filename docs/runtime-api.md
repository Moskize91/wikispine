...
`wikispine init` 安装 runtime 数据包。默认下载地址由 `config/runtime-data.json` 中的 Hugging Face dataset repo、revision 和 artifact 推导，也可以指定镜像 URL 或本地 ZIP 文件：
...
所有安装来源都必须通过 `config/runtime-data.json` 中记录的 ZIP MD5 校验；CLI 不提供覆盖 MD5 的参数。当前 artifact 和 MD5 仍是占位值，正式 runtime 数据包发布后需要更新该配置文件。
